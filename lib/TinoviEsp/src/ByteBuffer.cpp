...
//ByteBuffer::ByteBuffer(byte* d, unsigned int buf_size){
//	data = d;
//	capacity = buf_size;
//	position = 0;
//	length = 0;
...
void ByteBuffer::init(unsigned int buf_size){
...
void ByteBuffer::init(byte* d, unsigned int buf_size){
	data = d;
	capacity = buf_size;
	position = 0;
	length = 0;
...
void ByteBuffer::reset(unsigned int buf_size){
	capacity = buf_size;
	position = 0;
	length = 0;
}
...
uint32_t ByteBuffer::getRUInt32(){
	uint32_t ret;
    byte *pointer = (byte *)&ret;
	pointer[3] = get();
	pointer[2] = get();
